...
- [ ] 5.1 Add a `comment:` block to `.codecov.yml` so each PR gets an automatic Codecov comment showing the per-file coverage diff. Use a simple layout (`reach, diff, flags, files`) and `require_changes: false` so the comment posts even when the diff doesn't move coverage (useful for confirming "yes I checked").
